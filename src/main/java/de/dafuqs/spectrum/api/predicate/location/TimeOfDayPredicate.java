...
public record TimeOfDayPredicate(@Nullable TimeHelper.TimeOfDay name, MinMaxBounds.Ints range) {
	...
	private static final Codec<TimeOfDayPredicate> VALUED_CODEC = Codec.INT.xmap(t -> new TimeOfDayPredicate(null, MinMaxBounds.Ints.exactly(t)), p -> p.range.min().orElse(0));
	...
	).map(range -> new TimeOfDayPredicate(null, range), TimeOfDayPredicate::range);
