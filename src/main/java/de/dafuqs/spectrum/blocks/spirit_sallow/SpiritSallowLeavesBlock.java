...
public class SpiritSallowLeavesBlock extends LeavesBlock {
	...
	public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
		...
		if(/*!state.get(LeavesBlock.PERSISTENT) && state.get(LeavesBlock.DISTANCE) > 1 && world.getBlockState(pos.up()).isAir()*/ true) {
			double startX = pos.getX() + random.nextFloat();
			double startY = pos.getY() + 1.01;
			double startZ = pos.getZ() + random.nextFloat();
			
			double velocityX = 0.02 - random.nextFloat() * 0.06;
			double velocityY = 0.02 + random.nextFloat() * 0.04;
			double velocityZ = 0.02 - random.nextFloat() * 0.06;
