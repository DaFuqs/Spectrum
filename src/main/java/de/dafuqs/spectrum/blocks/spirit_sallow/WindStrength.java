...
public class WindStrength {
	...
	private SimplexNoiseSampler[] octaveSamplers = new SimplexNoiseSampler[1];
	...
	public Vec3d getWindStrength(long tick, Random random) {
		...
		if (tick != cachedTick) {
			cachedValue = new Vec3d(octaveSamplers[0].sample(0, 0, (tick + MinecraftClient.getInstance().getTickDelta()) / 512D),
