...
public class AmethystChestBlockEntityRenderer<AmethystChestBlockEntity extends SpectrumChestBlockEntity> implements BlockEntityRenderer<AmethystChestBlockEntity> {

	private static final SpriteIdentifier spriteIdentifier = new SpriteIdentifier(SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE, new Identifier(SpectrumCommon.MOD_ID, "block/amethyst_chest"));
	private final ModelPart root;

	public AmethystChestBlockEntityRenderer(BlockEntityRendererFactory.Context ctx) {
		TexturedModelData texturedModelData = getTexturedModelData();
		root = texturedModelData.createModel();
	}

	@Override
	public void render(AmethystChestBlockEntity entity, float tickDelta, MatrixStack matrixStack, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		matrixStack.push();
