...
public class AltarCraftingCategory<R extends AltarCraftingRecipe> implements DisplayCategory<AltarCraftingRecipeDisplay<R>> {
    ...
    public Text getTitle() {
        return new TranslatableText("container.spectrum.rei.altar_crafting.title");
    }

    @Override
    public Renderer getIcon() {
        return EntryStacks.of(SpectrumBlocks.ALTAR);
    }
    ...
    public List<Widget> setupDisplay(AltarCraftingRecipeDisplay display, Rectangle bounds) {
        ...
        if(!display.isUnlocked()) {
            widgets.add(Widgets.createLabel(new Point(startPoint.x - 6, startPoint.y + 33), new TranslatableText("container.spectrum.rei.altar.recipe_not_unlocked_line_1")).leftAligned().color(0x3f3f3f).noShadow());
            widgets.add(Widgets.createLabel(new Point(startPoint.x - 6, startPoint.y + 43), new TranslatableText("container.spectrum.rei.altar.recipe_not_unlocked_line_2")).leftAligned().color(0x3f3f3f).noShadow());
            return widgets;
        }
        ...
        if(display.craftingTime == 20) {
            text = new TranslatableText("container.spectrum.rei.altar.crafting_time_one_second_and_xp", 1, display.experience);
        } else {
            text = new TranslatableText("container.spectrum.rei.altar.crafting_time_and_xp", (display.craftingTime / 20), display.experience);
        }
