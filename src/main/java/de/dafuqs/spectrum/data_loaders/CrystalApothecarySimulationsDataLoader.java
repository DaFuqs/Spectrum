...
public class CrystalApothecarySimulationsDataLoader extends SimpleJsonResourceReloadListener {
	...
	public record SimulatedBlockGrowthEntry(Block buddingBlock, Collection<Block> validNeighbors, int ticksForCompensationLootPerValidNeighbor, ItemStack compensatedStack) {
		
		public static final Codec<SimulatedBlockGrowthEntry> CODEC = RecordCodecBuilder.create(i -> i.group(
				BuiltInRegistries.BLOCK.byNameCodec().fieldOf("budding_block").forGetter(c -> c.buddingBlock),
	...
	protected void apply(Map<ResourceLocation, JsonElement> prepared, ResourceManager manager, ProfilerFiller profiler) {
		...
		prepared.forEach((identifier, jsonElement) -> {
			DataResult<Pair<Optional<SimulatedBlockGrowthEntry>, JsonElement>> result = ConditionalOps.createConditionalCodec(SimulatedBlockGrowthEntry.CODEC).decode(JsonOps.INSTANCE, jsonElement.getAsJsonObject());
			// ConditionalOps.createConditionalCodec makes the data loader respect load conditions. Hooooray!
			if (result.isSuccess() && result.getOrThrow().getFirst().isPresent()) {
				Block buddingBlock = result.getOrThrow().getFirst().get().buddingBlock;
				...
				COMPENSATIONS.put(buddingBlock, entry.result().get());
			}
