...
package de.dafuqs.spectrum.datafixer.quilt_dfu.impl;
...
public abstract class QuiltDataFixesInternals {
	...
	public static @NotNull QuiltDataFixesInternals get() {
		if (instance == null) {
			...
			try {
				latestVanillaSchema = Schemas.getFixer().getSchema(DataFixUtils.makeKey(SharedConstants.getGameVersion().getSaveVersion().getId()));
			} catch (Exception e) {
