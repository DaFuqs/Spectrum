...
public interface PigmentEnergyStorage {
	...
	int addEnergy(CMYKColor color, int amount);
    ...
    int drainEnergy(CMYKColor color, int requestedAmount);
    ...
    boolean requestEnergy(CMYKColor color, int requestedAmount);
	

	// gets the amount of stored energy of that type
    int getEnergy(CMYKColor color);
