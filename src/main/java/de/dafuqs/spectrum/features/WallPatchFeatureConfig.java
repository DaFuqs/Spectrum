...
public class WallPatchFeatureConfig implements FeatureConfig {
	...
	public static final Codec<WallPatchFeatureConfig> CODEC = RecordCodecBuilder.create((instance) -> instance.group(Registry.BLOCK.getCodec().fieldOf("block").forGetter((config) -> config.block), Codec.intRange(1, 64).fieldOf("search_range").orElse(10).forGetter((config) -> config.searchRange), Codec.BOOL.fieldOf("can_place_on_floor").orElse(false).forGetter((config) -> config.placeOnFloor), Codec.BOOL.fieldOf("can_place_on_ceiling").orElse(false).forGetter((config) -> config.placeOnCeiling), Codec.BOOL.fieldOf("can_place_on_wall").orElse(false).forGetter((config) -> config.placeOnWalls), IntProvider.NON_NEGATIVE_CODEC.fieldOf("width").forGetter((config) -> config.width), IntProvider.NON_NEGATIVE_CODEC.fieldOf("height").forGetter((config) -> config.height), RegistryCodecs.entryList(Registry.BLOCK_KEY).fieldOf("can_be_placed_on").forGetter((config) -> config.canPlaceOn)).apply(instance, WallPatchFeatureConfig::new));
