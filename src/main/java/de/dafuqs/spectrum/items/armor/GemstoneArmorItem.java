		...
		switch (type) {
			case HELMET -> {
				if (source.getEntity() instanceof LivingEntity livingSource) {
					livingSource.addEffect(new MobEffectInstance(MobEffects.WEAKNESS, 5 * 20, SpectrumConfig.CONFIG.GemstoneArmorWeaknessAmplifier.get()));
					livingSource.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, 5 * 20, SpectrumConfig.CONFIG.GemstoneArmorSlownessAmplifier.get()));
				}
			}
			case CHESTPLATE -> {
				targetEntity.addEffect(new MobEffectInstance(MobEffects.ABSORPTION, 5 * 20, SpectrumConfig.CONFIG.GemstoneArmorAbsorptionAmplifier.get()));
				targetEntity.addEffect(new MobEffectInstance(MobEffects.DAMAGE_RESISTANCE, 5 * 20, SpectrumConfig.CONFIG.GemstoneArmorResistanceAmplifier.get()));
			}
			case LEGGINGS -> {
				targetEntity.addEffect(new MobEffectInstance(MobEffects.REGENERATION, 5 * 20, SpectrumConfig.CONFIG.GemstoneArmorRegenerationAmplifier.get()));
			}
			case BOOTS -> {
				targetEntity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 5 * 20, SpectrumConfig.CONFIG.GemstoneArmorSpeedAmplifier.get()));
				targetEntity.addEffect(new MobEffectInstance(MobEffects.INVISIBILITY, 5 * 20, 0));
			}
		}
