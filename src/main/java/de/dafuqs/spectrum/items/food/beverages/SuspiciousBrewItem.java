...
public class SuspiciousBrewItem extends BeverageItem {
	...
	public void appendTooltip(ItemStack itemStack, World world, List<Text> tooltip, TooltipContext tooltipContext) {
		...
		if(BeverageItem.isPreviewStack(itemStack)) {
			tooltip.add(new TranslatableText("item.spectrum.suspicious_brew.tooltip.preview").formatted(Formatting.GRAY));
			tooltip.add(new TranslatableText("item.spectrum.suspicious_brew.tooltip.preview2").formatted(Formatting.GRAY));
		}
	}
