...
public class EnchantmentCanvasItem extends Item {
	...
	public void appendTooltip(ItemStack stack, World world, List<Text> tooltip, TooltipContext context) {
		...
		if (boundItem.isPresent()) {
			tooltip.add(new TranslatableText("item.spectrum.enchantment_canvas.tooltip.bound_to").append(boundItem.get().getName()));
		} else {
			tooltip.add(new TranslatableText("item.spectrum.enchantment_canvas.tooltip.not_bound"));
			tooltip.add(new TranslatableText("item.spectrum.enchantment_canvas.tooltip.not_bound2"));
		}
