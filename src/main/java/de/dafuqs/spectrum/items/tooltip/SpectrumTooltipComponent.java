...
public class SpectrumTooltipComponent implements TooltipComponent {
	...
	public void drawOutline(int x, int y, int columns, int rows, MatrixStack matrices, int z, TextureManager textureManager) {
		...
		int j;
		for (j = 0; j < columns; ++j) {
			this.draw(matrices, x + 1 + j * 18, y, z, textureManager, Sprite.BORDER_HORIZONTAL_TOP);
			this.draw(matrices, x + 1 + j * 18, y + rows * 20, z, textureManager, Sprite.BORDER_HORIZONTAL_BOTTOM);
		}

		for (j = 0; j < rows; ++j) {
			this.draw(matrices, x, y + j * 20 + 1, z, textureManager, Sprite.BORDER_VERTICAL);
			this.draw(matrices, x + columns * 18 + 1, y + j * 20 + 1, z, textureManager, Sprite.BORDER_VERTICAL);
		}
	...
	public void drawDottedSlot(int x, int y, MatrixStack matrices, int z, TextureManager textureManager) {
