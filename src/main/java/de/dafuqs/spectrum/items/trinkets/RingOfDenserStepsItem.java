...
public class RingOfDenserStepsItem extends InkDrainTrinketItem implements GravitableItem {
	...
	public Multimap<EntityAttribute, EntityAttributeModifier> getModifiers(ItemStack stack, SlotReference slot, LivingEntity entity, UUID uuid) {
		...
		double knockbackResistance = getExtraKnockbackResistance(storedInk);
		if (knockbackResistance != 0) {
			modifiers.put(EntityAttributes.GENERIC_KNOCKBACK_RESISTANCE, new EntityAttributeModifier(uuid, "spectrum:ring_of_denser_steps", knockbackResistance, EntityAttributeModifier.Operation.ADDITION));
		}

		return modifiers;
	}

	public double getExtraKnockbackResistance(long storedInk) {
		if (storedInk < 100) {
			return 0;
		} else {
	...
	public float getGravityMod(ItemStack stack) {
		FixedSingleInkStorage inkStorage = getEnergyStorage(stack);
		long storedInk = inkStorage.getEnergy(inkStorage.getStoredColor());
		return (float) getExtraKnockbackResistance(storedInk) / 100;
	}
