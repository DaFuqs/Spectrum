...
public abstract class ChunkNoiseSamplerMixin {
	...
	@Inject(method = "<init>", at = @At(value = "INVOKE", target = "Lnet/minecraft/world/level/levelgen/NoiseGeneratorSettings;oreVeinsEnabled()Z"))
	public void spectrum$init(int cellCountXZ, RandomState random, int firstNoiseX, int firstNoiseZ, NoiseSettings noiseSettings, DensityFunctions.BeardifierOrMarker beardifier, NoiseGeneratorSettings noiseGeneratorSettings, Aquifer.FluidPicker fluidPicker, Blender blendifier, CallbackInfo ci, @Local(ordinal = 0) NoiseRouter noiseRouter, @Local ImmutableList.Builder<BlockStateFiller> builder) {
